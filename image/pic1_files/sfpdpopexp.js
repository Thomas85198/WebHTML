...
var u = navigator.userAgent, ua = navigator.userAgent.toLowerCase(), isLineApp = u.indexOf("Line") > -1, isFbApp = u.indexOf("FBIOS;FBDV") > -1, isWeixinApp = ua.match(/MicroMessenger/i) == "micromessenger";
...
function showPdPopupAd_pdup() {
	...
	for (var i=0; i<sf_temp_pdup.length; i++) {
        ...
        if (this_referrer.indexOf("pilio.idv")!=-1 || this_referrer.indexOf("viablog.okmall.tw")!=-1) {
			...
			list_pdup_html=list_pdup_html+'<div id="closebtn'+i+'_pdup" style="position:absolute;z-index:9999;width:60px;height:60px;right:-3px;top:-3px;display:none;">';
				...
				if (!event.isEmpty) {
					...
					if (check_passback.querySelector("div>script") && check_passback.querySelector("div>script").innerText.indexOf("definePassback('/7682122/")>0) {
						if ((check_passback.querySelector("div>div>div") && check_passback.querySelector("div>div>div").childNodes.length>0) || (check_passback.querySelector("div>div>iframe"))) {
							setTimeout("document.getElementById('lightbox0_pdup').style.cssText='background: rgba(0,0,0,0.7);z-index: 99999;width: 100%;height: 100%;padding: 0px;position: fixed;top: 0px;display: flex;justify-content: center;align-items: center;';",1500);
